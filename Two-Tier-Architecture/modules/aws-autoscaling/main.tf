resource "aws_launch_template" "Web-LC" {
  ...
  key_name      = "vockey" #ใช้key pair "labuser.pem" ในการsshเข้า
...
resource "aws_autoscaling_group" "Web-ASG" {
  name                      = var.asg-name
  vpc_zone_identifier       = [data.aws_subnet.public-subnet1.id, data.aws_subnet.public-subnet2.id]  # ← เปลี่ยนตรงนี้
  launch_template {
